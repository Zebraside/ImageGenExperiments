...
2. **`lit_module.LoRADiffusionModule`** — the training objective lives in `_diffusion_loss`: VAE-encode image → latents, add noise at a random timestep, predict noise (`epsilon`) or velocity (`v_prediction`, switched on `bundle.prediction_type`), MSE loss with optional min-SNR-γ weighting (`train.snr_gamma`). Only `bundle.trainable_parameters()` get optimizer state. `validation_step` reuses `_diffusion_loss` with a per-batch deterministic generator (fixed noise/timesteps) so `val/loss` is comparable across steps and can drive best-checkpoint selection. `save_weights()` writes portable LoRA adapter safetensors in `lora` mode, or a full pipeline in `full` mode.
3. **`data/datamodule.ImageFolderDataModule`** wraps **`data/dataset.ImageFolderDataset`** — recursively loads images under `data.root`, normalizes to `[-1, 1]` (VAE convention), and resolves the per-image sidecar caption with fallback to the trigger prompt. When `data.val_size` is set it carves off a deterministic held-out validation split and exposes a `val_dataloader`.
4. **`callbacks.SampleImageCallback`** — periodically samples from the trigger prompt (step- or epoch-based), saves to `output_dir/samples`, and logs to W&B when active. It hooks the training loop (the step gate is accumulation-safe). Checkpointing: with a validation split, `train.py` keeps the best checkpoint by `val/loss` (plus `last.ckpt`); without one it falls back to step-based `train/loss` selection.
